...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Июн 06 2023 г., 07:29
...
--
-- База данных: `data_base1`
--
CREATE DATABASE IF NOT EXISTS `data_base1` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `data_base1`;
...
INSERT INTO `sub_order` (`id`, `storage_id`, `order_id`, `count`, `sum`) VALUES
(17, 10, 18, 1, 150),
(19, 22, 18, 1, 0);
...
CREATE TRIGGER `insert_sub_order` BEFORE INSERT ON `sub_order` FOR EACH ROW BEGIN
 IF (SELECT storage.pharmacy_id FROM worker, orders, storage 
        WHERE worker.id = orders.worker_id 
        AND NEW.order_id = orders.id 
        AND NEW.storage_id = storage.id 
        AND storage.pharmacy_id = worker.pharmacy_id) IS NOT NULL THEN
...
INSERT INTO `worker` (`id`, `login`, `password`, `user_hash`, `ip`, `fio`, `num_phone`, `image`, `rol`, `pharmacy_id`) VALUES
(1, 'krasilkoff', '0773f7e5d21714681d9ff4394c6d4997', '55acbf8f8e94a5443f78b9c995c3d080', 2130706433, 'Красильников В. И.', '89504677438', NULL, 'admin', 1),
(2, 'test', 'e08a7c49d96c2b475656cc8fe18cee8e', '', 0, 'Цыгвинцев  О.С.', '89504677438', NULL, 'worker', 2),
(3, 'krasilkoff5543', '2f938838242b09f44e1cef6497f02c45', '', 0, 'Длввыыфыыф', '53333333333', NULL, 'worker', 1);
...
ALTER TABLE `sub_order`
  ADD PRIMARY KEY (`id`),
  ADD KEY `order_id` (`order_id`) USING BTREE,
  ADD KEY `sub_order_ibfk_3` (`storage_id`);
...
ALTER TABLE `sub_order`
  ADD CONSTRAINT `sub_order_ibfk_2` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `sub_order_ibfk_3` FOREIGN KEY (`storage_id`) REFERENCES `storage` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
